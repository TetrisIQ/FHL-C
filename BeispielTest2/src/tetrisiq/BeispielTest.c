...
int capsFile(char* filename) {
    ...
    fclose(file);
    if (grossBuchstabe > kleinBuchstabe) {
        return 1;
    } else {
        return 0;
    }
...
//    printf("%c is max", findMax("aBBccDDeeFFggghhhhhhhhh"));
    int value[] = {
    42
    };
    printf("%f avg \n", avgArray(value, 1));

    printf("exits: %d", exists(value, 1, is42));

    printf("\nCaps FIle: %d", capsFile("/home/alex/test.txt"));
    printf("\n");
    Listnode ln;
    addFront(&ln, 1);
//    addFront(&ln, 10);
    printf("\nListnode: %d", maxList(&ln));
    printf("\n");
